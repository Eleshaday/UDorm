...
export default function ListPropertyPage() {
  async function submitMock() {
    const listing = {
      ...
      bathrooms: Number((document.getElementById('bathrooms') || {}).value || 1),
      price: Number((document.getElementById('monthly-rent') || {}).value || 0),
      available: (document.getElementById('available-date') || {}).value,
      distance: (document.getElementById('campus-distance') || {}).value,
      images: [],
      amenities: [],
    ...
    if (res.ok) alert('Listing submitted!')
    else alert('You must be logged in to list a property.')
  }
  return (
    <main className={styles.listingFormContainer}>
      ...
      <div className={styles.formActions}>
        ...
        <button onClick={submitMock} className={`${styles.btn} ${styles.btnPrimary}`}>Submit</button>
      </div>
    </main>
  )
}
